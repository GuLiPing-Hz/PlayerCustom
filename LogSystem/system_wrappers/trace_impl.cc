  ...
  if (file_name_utf8) {
    if (add_file_counter) {
      ...
      CreateFileName(file_name_utf8, file_name_with_counter_utf8,
                     file_count_text_);
      if (trace_file_.OpenFile(file_name_with_counter_utf8,true, false, false,
                               true) == -1) {
        return -1;
      }
    } else {
      file_count_text_ = 0;
      if (trace_file_.OpenFile(file_name_utf8,true, false, false, true) == -1) {
        return -1;
      }
    }
  }
    ...
    if (trace_file_.Open()) {
      if (row_count_text_ > WEBRTC_TRACE_MAX_FILE_SIZE) {
        ...
        if (file_count_text_ == 0) {
          ...
          if (trace_file_.OpenFile(new_file_name,true, false, false,
                                   true) == -1) {
            return;
          }
        }
...
int32_t Trace::SetTraceFile(const char* file_name,const bool add_file_counter) 
{
  TraceImpl* trace = TraceImpl::GetTrace();
  if (trace) 
  {

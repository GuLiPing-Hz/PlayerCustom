...
bool ConditionVariableEventWin::SleepCS(CriticalSectionWrapper& crit_sect,
  ...
  ++(num_waiters_[eventID]);//进入等待，等待者加一
  LeaveCriticalSection(&num_waiters_crit_sect_);

  CriticalSectionWindows* cs = static_cast<CriticalSectionWindows*>(&crit_sect);
  //LeaveCriticalSection(&cs->crit);
  cs->Leave();//等待的时候需要离开临界区，这样可以使上层可以释放锁
  HANDLE events[2];
  ...
  EnterCriticalSection(&num_waiters_crit_sect_);
  --(num_waiters_[eventID]);//等待者减一

  // Last waiter should only be true for WakeAll(). WakeAll() correspond
  // to position 1 in events[] -> (result == WAIT_OBJECT_0 + 1)
  const bool last_waiter = (result == WAIT_OBJECT_0 + 1) &&(num_waiters_[eventID] == 0);//如果是第二个事件，并且没有等待者了
  LeaveCriticalSection(&num_waiters_crit_sect_);

  if (last_waiter) {
    // Reset/unset the WakeAll() event since all threads have been released.
    ResetEvent(events_[eventID]);//手动重置事件
  }

  //EnterCriticalSection(&cs->crit);
  cs->Enter();//等待结束后仍需要重新进入临界区
  return ret_val;
}

void ConditionVariableEventWin::Wake() {
  EnterCriticalSection(&num_waiters_crit_sect_);
  const bool have_waiters = (num_waiters_[WAKEALL_0] > 0) ||(num_waiters_[WAKEALL_1] > 0);//如果有等待者
  LeaveCriticalSection(&num_waiters_crit_sect_);

  if (have_waiters) {
    SetEvent(events_[WAKE]);//启动一个等待事件
  }
}
...
void ConditionVariableEventWin::WakeAll() {
  ...
  //==0的时候用1其实也是为了代码的简洁
  eventID_ = (WAKEALL_0 == eventID_) ? WAKEALL_1 : WAKEALL_0;//防止新加入的等待因为后面释放的事件而启动，所以先换一个等待的事件

  // Trigger current event
  const EventWakeUpType eventID = eventID_;
  const bool have_waiters = num_waiters_[eventID] > 0;//查看是否有等待者
  LeaveCriticalSection(&num_waiters_crit_sect_);

  if (have_waiters) {
    SetEvent(events_[eventID]);//启动所有的等待的事件
  }
}

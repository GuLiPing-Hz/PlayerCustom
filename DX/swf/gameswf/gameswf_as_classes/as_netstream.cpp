...
//		printf("push status: %s %s\n", level, code);
	...
	bool as_netstream::open_stream(const char* c_url)
	{
		...
		if(avformat_open_input(&m_FormatCtx, c_url, NULL, NULL) < 0)
...
//			log_error("Couldn't open file '%s'\n", c_url);
		...
		m_convert_ctx = sws_getContext(
			...
			m_VCodecCtx->width, m_VCodecCtx->height, PIX_FMT_RGBA,
	...
	// it is running in sound mixer thread
	bool as_netstream::decode_audio(AVPacket& pkt, Sint16** data, int* size)
	{
		bool ok = false;
		int frame_size;
		Uint8* decoder_buf = (Uint8*) malloc((AVCODEC_MAX_AUDIO_FRAME_SIZE * 3) / 2);
		if (avcodec_decode_audio3(m_ACodecCtx, (Sint16*) decoder_buf, &frame_size,	&pkt) >= 0)
		{
	...
	Uint8* as_netstream::decode_video(const AVPacket& pkt)
	{
		...
		if (got)
		{
			...
			int size = avpicture_get_size(PIX_FMT_RGBA, m_VCodecCtx->width, m_VCodecCtx->height);
			Uint8* data = (Uint8*) malloc(size);

			AVPicture picture1;
			avpicture_fill(&picture1, data, PIX_FMT_RGBA, m_VCodecCtx->width, m_VCodecCtx->height);

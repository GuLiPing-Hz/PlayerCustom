				...
				switch (m_parent->m_format)
				{
					...
					case sound_handler::FORMAT_MP3:
					{
						...
						while (m_decoded < m_parent->m_size)
						{
							uint8_t* frame;
							int framesize;
							int decoded = av_parser_parse2(m_parser, m_cc, &frame, &framesize,
								m_parent->m_data + m_decoded , m_parent->m_size - m_decoded, 0, 0,0);
							...
							if (framesize > 0)
							{
								...
								if (avcodec_decode_audio3(m_cc, (int16_t*) (buf + buflen), &len, &packet/*frame, framesize*/) >= 0)
								{
									buflen += len;

...
namespace gameswf
{
	typedef as_object* (*gameswf_module_init)(player* player, const array<as_value>& params);
	...
	struct player : public ref_counted
	{
		...
		root*  load_file(const char* filename);
		...
		const bool get_force_realtime_framerate() const;
		void set_force_realtime_framerate(const bool force_realtime_framerate);
		bool get_log_bitmap_info() const { return m_log_bitmap_info; }
		void set_log_bitmap_info(bool log_bitmap_info) { m_log_bitmap_info = log_bitmap_info; }
